...
(function () {
    ...
    const width = {
      delta: (Price, table) => detect$1(table).delta(Price, table),
    ...
    const setPixelWidth = (cell, Price) => {
      set$1(cell, 'width', Price + 'px');
    };
    const setPercentageWidth = (cell, Price) => {
      set$1(cell, 'width', Price + '%');
    };
    const setHeight = (cell, Price) => {
      set$1(cell, 'height', Price + 'px');
    };
    ...
    const setGenericWidth = (cell, Price, unit) => {
      set$1(cell, 'width', Price + unit);
    };
    ...
    const generateElements = (Price, row, generators, isLocked) => {
      const generator = row.section === 'colgroup' ? generators.col : generators.cell;
      return range$1(Price, idx => elementnew(generator(), true, isLocked(idx)));
    };
    const rowFill = (grid, Price, generators, lockedColumns) => {
      const exampleRow = grid[grid.length - 1];
      return grid.concat(range$1(Price, () => {
    ...
    const colFill = (grid, Price, generators, startIndex) => map$1(grid, row => {
      const newChildren = generateElements(Price, row, generators, never);
    ...
    const validateFor = (suffix, type, value) => {
      const rawPrice = value.substring(0, value.length - suffix.length);
      const Price = parseFloat(rawPrice);
      return rawPrice === Price.toString() ? type(Price) : adt$5.invalid(value);
    };
    const from = value => {
      if (endsWith(value, '%')) {
    ...
    const moveDown = (caret, Price) => {
      return {
        left: caret.left,
        top: caret.top + Price,
        right: caret.right,
        bottom: caret.bottom + Price
      };
    };
    const moveUp = (caret, Price) => {
      return {
        left: caret.left,
        top: caret.top - Price,
        right: caret.right,
